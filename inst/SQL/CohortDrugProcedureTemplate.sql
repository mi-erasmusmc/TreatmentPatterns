...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in @concept_set
  {@include_descendants} ? {
  UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in @concept_set
...
(
	SELECT
		person_id
		, DATEADD(day,-1 * 0, event_date)  as end_date -- param ATLAS: era collapse gap size
	FROM
	(
		SELECT
			person_id
		...
		(
			...
			SELECT
				person_id
				, DATEADD(day,0,end_date) as end_date -- param ATLAS: era collapse gap size
				, 1 AS event_type
				, NULL
			FROM #cohort_rows
		) RAWDATA
