...
#' 
#' Optional, only for OMOP-CDM data: Perform baseline characterization of 
#' study/target population.
#'
#' @param dataSettings
#'     Settings object as created by createDataSettings().
#' @param characterizationSettings
#'     Settings object as created by createCharcterizationSettings().
#' @param saveSettings
#'     Settings object as created by createSaveSettings().
#'
#' @export
cohortCharacterization <- function(
    dataSettings,
  ...
  if (!file.exists(file.path(saveSettings$outputFolder, "characterization"))) {
    dir.create(
      path = file.path(saveSettings$outputFolder, "characterization"), 
      recursive = TRUE)
  }
  ...
  targetCohortIds <- cohortsToCreate$cohortId[
    cohortsToCreate$cohortType == "target"]
  ...
  customCovariates <- baselineCovariates[
    baselineCovariates$covariateId == "Custom", ]

  if (nrow(customCovariates) != 0) {
    for (c in 1:length(customCovariates$covariateName)) {
      baselineCovariates[
        baselineCovariates$covariateName == customCovariates$covariateName[c], 
        "covariateId"] <- as.character((999000 + c) * 1000 + 999)
    }

    customCovariateSettings <- createCustomCovariateSettings(
      list_covariates = customCovariates$covariateName)

    covariateSettings <- list(
      characterizationSettings$standardCovariateSettings,
  ...
  exportCharacterization(
    ...
    incremental = FALSE,
    covariateValueFileName = file.path(
      saveSettings$outputFolder, "characterization", "covariate_value.csv"),
    covariateRefFileName = file.path(
      saveSettings$outputFolder, "characterization", "covariate_ref.csv"),
    analysisRefFileName = file.path(
      saveSettings$outputFolder, "characterization", "analysis_ref.csv"),
    counts = cohortCounts,
    minCellCount = characterizationSettings$minCellCount)

  # Selection of standard results
  characterization <- readr::read_csv(
    file = file.path(
      saveSettings$outputFolder, 
      "characterization", "covariate_value.csv"), 
    col_types = list("i", "c", "d", "d", "c"))

  colnames(characterization) <- c("cohortId", "covariateId", "mean",
                                  "sd", "databaseId") 
  ...
  if (characterizationSettings$returnCovariates == "all") {
    names <- readr::read_csv(
      file = file.path(
        saveSettings$outputFolder, 
        "characterization", "covariate_ref.csv"),
      col_types = list("c", "c", "i", "i"))

    colnames(names) <- c("covariateId", "covariateName",
                         "analysisId", "conceptId")

    characterization <- merge(
      characterization,
      names[, c("covariateId", "covariateName")],
      by = "covariateId")

  } else if (characterizationSettings$returnCovariates == "selection") {
    characterization <- merge(
      x = baselineCovariates[, c("covariateId", "covariateName")],
      y = characterization,
      by = "covariateId")
  } else {
    stop(paste(
      "characterizationSettings$returnCovariates",
      "should be 'all' or 'selection'"))
  }

  # Add cohort counts
  characterization <- rbind(
    characterization,
    cbind(
      covariateId = "Custom",
      covariateName = "Number of persons",
      cohortId = cohortCounts$cohortId,
      mean = cohortCounts$cohortEntries,
      sd = NA,
      databaseId = saveSettings$databaseName))
