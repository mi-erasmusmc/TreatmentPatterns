...
This vignette describes how to execute a custom treatment patterns study in five steps:
...
- Event cohort(s) (= treatment(s) of interest). You need to define a cohort for each treatment of interest (e.g. prescriptions of drugs, therapies, other treatments).
...
### The other event cohorts (metorolol, amlodipine, lisinopril, losartan) are defined similarly 
```

# Step 2: (optional, only for OMOP-CDM data) specify baseline characteristics of interest (characterizationSettings)

Determine which covariates are of interest for baseline characterization and select these from the large set of pre-defined covariates as defined by *standardCovariateSettings* (as defined by createCovariateSettings function in FeatureExtraction package) or create (a) custom covariate(s) using SQL. 

Define an object selecting which covariates we want to present in the summary. When using a characteristic of interest from FeatureExtraction we need to make sure it is included in *standardCovariateSettings*. For the example in this vignette we use the following default characteristics:
...
These settings allow the user to influence how the events of interest should be processed to form treatment pathways. See the table below for a list of parameters, with default values and description. For the example in this vignette we use the default settings:
...
A) by using function arguments when calling package functions (recommended for beginners) or,
B) by referring to pre-specified setting files in a folder.

We give the code for both approaches for databases in 'OMOP CDM' and 'other' format for the example in this vignette. Try to run these examples to become familiar with the package. 
